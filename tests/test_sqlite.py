...
# Sqlite filename/location can be replaced using a ENV variable named "TEST_SQLITE_FILENAME"
CONFIG = {  # These settings can be replaced using ENV variables with the same names
    "TEST_SQLITE_FILENAME": ":memory:",
    # "TEST_SQLITE_FILENAME": "PythonUnitTest.sqlite",
    "TEST_SQLITE_CLEAR": True,
}

for key in CONFIG.keys():
    try:
        if key == "TEST_SQLITE_CLEAR":
            try:
...
class TestSqlite(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.db = SqliteDB(filename=CONFIG["TEST_SQLITE_FILENAME"])
    ...
    def tearDownClass(cls):
        if CONFIG["TEST_SQLITE_CLEAR"]:
            log.info(f"{len(cls.saved_stops)} stops generated: {cls.saved_stops}")
            log.info("Removing these stops from SQLite DB...")
            deleted_stops = list()
            ...
            log.warning("WARNING: Stops generated and saved on SQLite DB WILL NOT BE CLEARED")
        cls.db.disconnect()

    def test_connection(self):
        self.assertEqual(type(self.db.db), sqlite3.Connection)
    ...
    def test_get_stop_auto_save(self):
        """Search a Stop that does not exist on the SQLite DB, but is available on an Online getter.
        Use the PyBuses auto_save option. Check if the stop is saved after searching it.
        """
        ...
        # Stop should be saved on SQLite DB - check that ID and name are the same
        saved = self.db.is_stop_saved(stopid)
        stop_sqlite = self.db.find_stop(stopid)
        self.assertTrue(saved)
        self.assertEqual(stop_pybuses.stopid, stop_sqlite.stopid)
        self.assertEqual(stop_pybuses.name, stop_sqlite.name)
    ...
    def test_get_stop_no_auto_save(self):
        ...
        # Stop should not be saved on SQLite DB
        saved = self.db.is_stop_saved(stopid)
    ...
    def test_stop_save_read_check_time(self):
        ...
        cursor.execute("SELECT saved, updated FROM stops WHERE id=?", (stopid,))
